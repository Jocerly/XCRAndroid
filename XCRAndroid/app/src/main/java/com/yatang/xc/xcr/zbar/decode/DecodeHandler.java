...
package com.yatang.xc.xcr.zbar.decode;
    ...
    private void decode(byte[] data, int width, int height) {
       // long start = System.currentTimeMillis();
        ...
        if (activity instanceof ScanActivity) {
            mCropRect = ((ScanActivity)activity).initCrop();
        } else if (activity instanceof ScanCodeActivity) {
            mCropRect = ((ScanCodeActivity)activity).initCrop();
        }
        String result =null;
        if (zBarDecoder != null) {
            try {
            ...
            if (activity instanceof ScanActivity) {
                handler = ((ScanActivity)activity).getHandler();
            } else if (activity instanceof ScanCodeActivity) {
                handler = ((ScanCodeActivity)activity).getHandler();
            }
            if (result != null) {
