package org.jocerly.jcannotation.widget.swiperefreshlayout;/*
...
/**
 * 自定义下拉刷新RefreshLayout，重写SwipeRefreshLayout代码，自定义动画效果
...
public class JCSwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    public static final int LARGE = SmileProgressDrawable.LARGE;
    // Maps to ProgressBar default style
    public static final int DEFAULT = SmileProgressDrawable.DEFAULT;
    ...
    private static final int MAX_ALPHA = 255;//设置透明度，0-255
    ...
    private static final int SCALE_DOWN_DURATION = 150;//消失的时间

    private static final int ALPHA_ANIMATION_DURATION = 300;

    private static final int ANIMATE_TO_TRIGGER_DURATION = 200;//开始转圈之前逗留时间
    ...
    SmileProgressDrawable mProgress;
    ...
    public void setSize(int size) {
        if (size != SmileProgressDrawable.LARGE && size != SmileProgressDrawable.DEFAULT) {
            return;
        }
        final DisplayMetrics metrics = getResources().getDisplayMetrics();
        if (size == SmileProgressDrawable.LARGE) {
     ...
     * @param attrs
     */
    public JCSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public JCSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void createProgressView() {
        ...
        mProgress = new SmileProgressDrawable(getContext(), this);
     ...
     * @param strokeWidth
     */
    public void setSmileStrokeWidth(float strokeWidth) {
    ...
    public void setSmileAnimationDuration(int animationDuration) {
        mProgress.setAnimationDuration(animationDuration);
    }
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        float adjustedPercent = (float) Math.max(dragPercent - .4, 0) * 5 / 3;// 1 可以下拉刷新
...
//        mProgress.setPercentage(tensionPercent * 2); // 最高值0.5//屏蔽该处，要不然下拉会打转
    ...
    private void finishSpinner(float overscrollTop) {
        if (overscrollTop > mTotalDragDistance) {
            ...
            // cancel refresh
            mRefreshing = false;
//            mProgress.setStartEndTrim(0f, 0f); // TODO: 2016/10/13 0013
    ...
    public interface OnChildScrollUpCallback {
         ...
         */
        boolean canChildScrollUp(JCSwipeRefreshLayout parent, @Nullable View child);
    }
}
